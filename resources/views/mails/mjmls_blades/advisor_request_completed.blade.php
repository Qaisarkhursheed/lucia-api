<?php /** @var \App\ModelsExtended\AdvisorRequest $AdvisorRequest **/ ?>
<mjml>
  <mj-head>
  ...
  <mj-column>
    ...
    <mj-section background-color="#fff">
      <mj-column>
        <mj-text mj-class="heading">Your request ( {{ $AdvisorRequest->request_title }} ) has been marked completed!</mj-text>
        <mj-text>Hey {{ $AdvisorRequest->user->first_name }}, please click the button below to rate your copilot.</mj-text>
                <mj-divider border-width="2px" border-color="white" />
                <mj-text font-size="16px" color="#BA886E" font-weight="700" align="center" font-family="Montserrat, monospace">
                    {{ $AdvisorRequest->advisor_assigned_copilot->user->first_name }}
                </mj-text>
                <mj-divider border-width="2px" border-color="white" />
                <mj-button width="100%"  href="{{ uiAppUrl( 'rate-my-copilot/' . $AdvisorRequest->id ) }}">Rate {{ $AdvisorRequest->advisor_assigned_copilot->user->first_name }}</mj-button>
